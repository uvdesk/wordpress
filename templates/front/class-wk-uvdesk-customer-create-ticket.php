<?php
/**
 * WK_UVDESK_Customer_Create_Ticket handler.
 *
...
if ( ! class_exists( 'WK_UVDESK_Customer_Create_Ticket' ) ) {
	/**
	 * WK_UVDESK_Customer_Create_Ticket class.
	 */
	class WK_UVDESK_Customer_Create_Ticket {
		/**
		 * Instance variable
		 *
		 * @var $instance
		...
		public function __construct() {
			add_shortcode( 'uvdesk', array( $this, 'wk_uvdesk_customer_create_ticket' ) );
		}
							...
							if ( ! wp_verify_nonce( sanitize_text_field( wp_unslash( filter_input( INPUT_POST, 'uvdesk_create_ticket_nonce', FILTER_SANITIZE_FULL_SPECIAL_CHARS ) ) ), 'uvdesk_create_ticket_nonce_action' ) ) {
								...
								if ( ! empty( $user ) ) {
									if ( isset( $_FILES['attachments']['size'][0] ) && 0 === $_FILES['attachments']['size'][0] ) {
										$ticket_status = Helper\WK_UVDESK_Api_Handler::wk_uvdesk_create_new_ticket( $post_data );
										...
										if ( in_array( $file_mime_type, $valid_file_mimes, true ) ) {
											$ticket_status = Helper\WK_UVDESK_Api_Handler::wk_uvdesk_create_new_ticket_with_attachement( $post_data, array_map( 'sanitize_text_field', $_FILES['attachments'] ) );
											if ( $ticket_status ) {
