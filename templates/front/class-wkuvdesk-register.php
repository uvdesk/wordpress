			...
			<div class="wk-uvdesk-signup-form">
				...
				<form method="POST" action="">
					...
					<div class="form-group">
						<p>
							<label for="cust_name"><?php esc_html_e( 'Name', 'uvdesk' ); ?></label>
							<input type="text" id="cust_name" name="user_name"/>
						</p>
					</div>
					<div class="form-group">
						<p>
							<label for="cust_email"><?php esc_html_e( 'Email', 'uvdesk' ); ?></label>
							<input type="email" id="cust_email" name="user_email">
						</p>
					</div>
					<div class="form-group">
						<p>
							<label for="cust_pass"><?php esc_html_e( 'Password', 'uvdesk' ); ?></label>
					...
					<?php
					if ( empty( $client_key ) ) {
						$client_key = esc_html__( 'Check for client Keys', 'uvdesk' );
					}
					?>
					<div class="g-recaptcha" data-sitekey="<?php echo esc_attr( $client_key ); ?>"></div>
					<?php wp_nonce_field( 'sign-up', 'signup-nonce' ); ?>
					<input name="action" type="hidden" value="<?php echo esc_attr( 'add user' ); ?>" />
					<input type="submit" name="signup_submit" value="<?php esc_attr_e( 'Submit', 'uvdesk' ); ?>" class="signup_submit">
				</form>

				<div class="text-center">
					<h3><?php esc_html_e( 'OR', 'uvdesk' ); ?></h3>
					<h4><?php esc_html_e( 'Already Have An Account?', 'uvdesk' ); ?>
						<a href="<?php echo esc_url( site_url() . '/uvdesk' ); ?>"><?php esc_html_e( 'Login', 'uvdesk' ); ?></a>
					</h4>
				</div>
			</div>
				<?php
