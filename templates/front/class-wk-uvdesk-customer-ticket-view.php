<?php
/**
 * WK_UVDESK_Customer_Ticket_View handler.
 *
...
if ( ! class_exists( 'WK_UVDESK_Customer_Ticket_View' ) ) {
	/**
	 * WK_UVDESK_Customer_Ticket_View class.
	 */
	class WK_UVDESK_Customer_Ticket_View {
		/**
		 * Instance variable
		 *
		 * @var $instance
		...
		public function __construct() {
			add_shortcode( 'uvdesk', array( $this, 'wk_uvdesk_customer_ticket_view' ) );
		}
		...
		public function wk_uvdesk_customer_ticket_view() {
			...
			if ( ! empty( $uvdesk_access_token ) ) {
				...
				if ( isset( $ticket_details->error ) || empty( $ticket_details ) ) {
					...
					$ticket_thread = Helper\WK_UVDESK_Api_Handler::wk_uvdesk_get_customer_data_api( 'ticket/' . $ticket . '/threads.json' );
					...
					<div class="uv-uvdesk-block-container wk-uvdesk-content-wrap">
						<div class="uv-uvdesk-pre-loader">
							<img <?php echo wp_kses_post( Includes\WK_UVDESK::wk_uvdesk_convert_attributes_to_html( $spinner_args ) ); ?> />
						</div>
								...
								<div class="ticket-create">
										<div class="thread-created-info">
											...
											<div class="msg-header">
												...
												echo '<img ' . wp_kses_post( Includes\WK_UVDESK::wk_uvdesk_convert_attributes_to_html( $thumbnail ) ) . ' />';
												?>
												</span>
												<span class="info">
																...
																foreach ( $ticket_details->createThread->attachments as $attchment_key => $attchment_value ) :
																	...
																	if ( in_array( $aname, $img_ar, true ) ) {
																		...
																		<a href="<?php echo esc_url( $wk_image ); ?>" target="_blank">
																				<img <?php echo wp_kses_post( Includes\WK_UVDESK::wk_uvdesk_convert_attributes_to_html( $wk_image ) ); ?> class="fa fa-file zip" title="<?php echo esc_attr( $anamea ); ?>" data-toggle="tooltip" data-original-title="<?php echo esc_attr( $attchment_value->name ); ?>">
											...
											?>
												<div class="thread-created-info">
													<div class="msg-header">
														...
														echo '<img ' . wp_kses_post( Includes\WK_UVDESK::wk_uvdesk_convert_attributes_to_html( $thumbnail ) ) . ' alt="' . esc_attr__( 'user-img', 'wk-uvdesk' ) . '" class="img-circle" />';
														?>
														</span>
														<span class="info">
															<span class="rpy-name">
									...
									<div class="msg-header">
										<span class="img-icon">
											...
											$thumbnail = isset( $ticket_details->ticket->customer->detail->customer->smallThumbnail ) && ! empty( $ticket_details->ticket->customer->detail->customer->smallThumbnail )
												...
												: 'https://cdn.uvdesk.com/uvdesk/images/e09dabf.png';
											echo wp_kses_post( '<img ' . wp_kses_post( Includes\WK_UVDESK::wk_uvdesk_convert_attributes_to_html( $thumbnail ) ) . '/>' );
											?>
										</span>
										<span class="info">
