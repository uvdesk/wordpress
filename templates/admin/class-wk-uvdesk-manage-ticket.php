<?php
/**
 * WK_UVDESK_Manage_Ticket handler.
 *
...
if ( ! class_exists( 'WK_UVDESK_Manage_Ticket' ) ) {
	/**
	 * WK_UVDESK_Manage_Ticket class.
	 */
	class WK_UVDESK_Manage_Ticket {
		/**
		 * Instance variable
		 *
		 * @var $instance
		 ...
		 * @return mixed
		 */
		public static function wk_uvdesk_manage_ticket() {
			if ( 'delete' === filter_input( INPUT_GET, 'action', FILTER_SANITIZE_FULL_SPECIAL_CHARS ) ) {
				...
				if ( ! empty( $post_id ) ) {
					$ids     = $post_id;
					$success = Helper\WK_UVDESK_Api_Handler::wk_uvdesk_delete_tag_ticket( 'ticket/' . $ids . '/trash.json' );
					if ( $success ) {
			...
			if ( filter_input( INPUT_GET, 'post', FILTER_SANITIZE_FULL_SPECIAL_CHARS ) ) {
				...
				printf(
					...
					esc_html__( 'Select a ticket first from', 'wk-uvdesk' ),
					esc_url( $admin_url ),
					esc_html__( 'here', 'wk-uvdesk' )
				);
				return;
			}
			$allowed_html = array(
			...
			if ( filter_input( INPUT_POST, 'uvdesk_thread_nonce', FILTER_SANITIZE_FULL_SPECIAL_CHARS ) ) {
					...
					if ( isset( $_FILES['attachments']['size'][0] ) && 0 === $_FILES['attachments']['size'][0] ) {
						...
						$thread_status = Helper\WK_UVDESK_Api_Handler::wk_uvdesk_post_thread_data_api(
							...
							array(
								...
								'actAsType'  => 'agent',
								'actAsEmail' => sanitize_text_field( wp_unslash( filter_input( INPUT_POST, 'agent_email', FILTER_SANITIZE_FULL_SPECIAL_CHARS ) ) ),
							)
						);
						...
						if ( in_array( $file_mime_type, $valid_file_mimes, true ) ) {
							...
							echo '<div class="alert alert-success alert-fixed  ">
								<span>
									...
									' . esc_html__( 'Please upload a valid file (PDF, DOC, DOCX, PNG, JPG, JPEG, GIF, ZIP, RAR) with maximum size of 20MB', 'wk-uvdesk' ) . '
								</span>
							</div>';
							?>
							<script>
				...
				<div class="updated notice">
					<p><?php echo isset( $thread_status->message ) ? esc_html( $thread_status->message ) : esc_html__( 'Contact technical team ...', 'wk-uvdesk' ); ?></p>
					</div>
						<?php
				}
			}
			...
			if ( ! empty( $uvdesk_access_token ) ) {
				$ticket_details = Helper\WK_UVDESK_Api_Handler::wk_uvdesk_get_customer_data_api( 'ticket/' . $ticket_id . '.json' );

				if ( isset( $ticket_details->error ) || empty( $ticket_details ) ) {
					echo wp_kses_post( '<h1>' . esc_html__( 'Invalid Details.', 'wk-uvdesk' ) . '</h1><h3>' . esc_html__( 'Please contact Administrator.', 'wk-uvdesk' ) . '</h3>' );
				} else {
						$ticket_thread    = Helper\WK_UVDESK_Api_Handler::wk_uvdesk_get_customer_data_api( 'ticket/' . $ticket_details->ticket->id . '/threads.json' );
						$data_api_members = Helper\WK_UVDESK_Api_Handler::wk_uvdesk_get_customer_data_api(
							'members.json',
							array(
								'sort'     => 'name',
								'fullList' => 'true',
					...
					<div>
						<header style="display:inline-block;">
							<h1>
							<?php
							esc_html_e( 'Ticket #', 'wk-uvdesk' );
							echo esc_attr( $ticket_details->ticket->incrementId );
							?>
							</h1>
							<input type='hidden' value="<?php echo esc_attr( $ticket_details->ticket->id ); ?>" class="uv-ticket-id">
							<a href = "<?php echo esc_url( admin_url( 'admin.php?page=uvdesk_ticket_system' ) ); ?>" class="button button-primary back-to-list"><?php esc_html_e( 'All tickets', 'wk-uvdesk' ); ?></a>
						</header>
					...
					<div class="wk-main-wrapper" >
						<div class="uv-tk-manage-wrapper">
							<div class="wk-cards tkt-intro">
								<h2>
									...
									echo( '<div style="display: inline-block;vertical-align: text-bottom;"><input type="radio" style="opacity: 0;" ><span class="wk-starred-ico ' . esc_attr( $select ) . '" data-id="' . esc_attr( $ticket_details->ticket->id ) . '" data-star-val="' . esc_attr( $str_no ) . '"></span></div>' );
									?>
									<?php echo esc_attr( $ticket_details->ticket->subject ); ?>
								<h2>
								<p>
									<span class="wk-space"><span class="wk-highlight"><?php esc_html_e( 'Created on -', 'wk-uvdesk' ); ?> </span><?php echo esc_html( $ticket_details->ticket->formatedCreatedAt ); ?></span>
									<span class="wk-space"><span class="wk-highlight"><?php esc_html_e( 'Agent - ', 'wk-uvdesk' ); ?></span>
									<select class="wk-sel-agent" data-id="<?php echo esc_attr( $ticket_details->ticket->id ); ?>">
										<option value=""><?php esc_html_e( 'Add agent', 'wk-uvdesk' ); ?></option>
										<?php
									...
									<span class="wk-space"><span class="wk-highlight"><?php esc_html_e( 'Priority -', 'wk-uvdesk' ); ?> </span>
									<select id="wk-sel-priority">
										<option value=""><?php esc_html_e( 'Add priority', 'wk-uvdesk' ); ?></option>
										<?php
										$priorities = array(
											1 => esc_html__( 'Low', 'wk-uvdesk' ),
											2 => esc_html__( 'Medium', 'wk-uvdesk' ),
											3 => esc_html__( 'High', 'wk-uvdesk' ),
											4 => esc_html__( 'Urgent', 'wk-uvdesk' ),
										);
										foreach ( $priorities as $id => $label ) {
											$selected = ( $id === $ticket_details->ticket->priority->id ) ? esc_attr( 'selected' ) : '';
						...
						<div class="wk-cards tkt-replay">
							...
							$thumbnail = isset( $ticket_details->ticket->customer->smallThumbnail ) && ! empty( $ticket_details->ticket->customer->smallThumbnail )
								...
								: esc_url( 'https://cdn.uvdesk.com/uvdesk/images/e09dabf.png' );
							$alt_text  = esc_url( 'https://cdn.uvdesk.com/uvdesk/images/e09dabf.png' ) === $thumbnail ? esc_attr__( 'Default Thumbnail', 'wk-uvdesk' ) : esc_attr__( 'Customer Thumbnail', 'wk-uvdesk' );
							echo '<img ' . wp_kses_post( Includes\WK_UVDESK::wk_uvdesk_convert_attributes_to_html( $thumbnail ) ) . ' alt="' . esc_attr( $alt_text ) . '" />';
							?>
							<span class="tkt-name">
							<?php
							echo wp_kses_post(
							...
							?>
							<span class="wk-accord"></span></span>
						</div>
													...
													<h4><strong> <?php esc_html_e( 'Uploaded files', 'wk-uvdesk' ); ?> </strong></h4>
														...
														foreach ( $thread_value->attachments as $attchment_key => $attchment_value ) :
															...
															<a href="<?php echo esc_url( $wk_image ); ?>" title="<?php echo esc_attr( $anamea ); ?>" target="_blank">
																<img <?php echo wp_kses( Includes\WK_UVDESK::wk_uvdesk_convert_attributes_to_html( $wk_image ), $allowed_html ); ?> class="fa fa-file zip"  data-toggle="<?php echo esc_attr( 'tooltip' ); ?>" data-original-title="<?php echo esc_attr( $attchment_value->name ); ?>"/>
															</a>
																<?php
															} elseif ( 'zip' === $aname ) {
																$attach_url = 'https://' . esc_attr( $domain ) . '.uvdesk.com/en/api/ticket/attachment/' . esc_attr( $aid ) . '.json?access_token=' . esc_attr( $access_token );
								...
								<?php
									...
									<div class="form-group wk-uvdesk-attachments">
										<div class="labelWidget">
											<input id="uv-uvdesk-attachments" class="fileHide" type="file" enableremoveoption="enableRemoveOption" decoratecss="attach-file" decoratefile="decorateFile" infolabeltext="<?php esc_attr_e( '+ Attach File', 'wk-uvdesk' ); ?>" infolabel="right" name="attachments[]">
			...
			<div class="uv-tk-manage-sidebar">
				<div class="wk-cards tkt-replay" style="margin-left: 0;">
					<h2><?php esc_html_e( 'Customer', 'wk-uvdesk' ); ?></h2>
						<?php
				...
				<div class="wk-cards tkt-replay" style="margin-left: 0;">
						...
						if ( isset( $ticket_details->ticket->customer->id ) ) {
							...
							$data_assign_api = Helper\WK_UVDESK_Api_Handler::wk_uvdesk_get_customer_data_api(
								'tickets.json',
								array(
						...
						?>
					<h2><?php esc_html_e( 'Total', 'wk-uvdesk' ); ?> <?php echo isset( $data_assign_api->pagination->totalCount ) ? esc_attr( $data_assign_api->pagination->totalCount ) : '0'; ?>  <?php esc_html_e( 'Tickets', 'wk-uvdesk' ); ?></h2>
						...
						<a style="text-decoration: none" href="<?php echo esc_url( '?page=uvdesk_ticket_system&action=view&post=' . esc_attr( $tkt_detail->incrementId ) ); ?>">
							<p><?php echo esc_html( $count ); ?>.
								...
								if ( $count > 2 ) {
									...
									echo "<a href='" . esc_url( $cust_url ) . "' ><h4>" . esc_html__( 'View All', 'wk-uvdesk' ) . '</h4></a>';
									break;
								}
								++$count;
							}
				...
				esc_html_e( 'Invalid setting details.', 'wk-uvdesk' );
			}
		}
	}
}
